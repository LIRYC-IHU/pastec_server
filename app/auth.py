...
async def decode_token(token: str, audience: str) -> dict:
    try:
        ...
        # Token JWT non loggé pour des raisons de sécurité
...
async def get_user_info(payload: dict = Depends(get_payload)) -> User:
    ...
    logger.debug(f"User ID: {payload.get('sub', 'unknown')}")
    try:
...
async def get_ai_model_info(payload: dict = Depends(get_payload)) -> AIModel:
    try:
        ...
        logger.debug(f"Client ID: {payload.get('sub', 'unknown')}")
...
async def get_refresh_token(refresh_token: str) -> dict:
    try:
        ...
        # Réponse non loggée pour des raisons de sécurité
...
def check_authorization(
    ...
    async def _check_authorization(payload: dict = Depends(get_auth_info)):
        ...
        logger.debug(f"Auth type: {payload.get('type', 'unknown')}")
