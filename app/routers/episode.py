...
@router.post("/{episode_id}/ai", status_code=202)
async def send_egm_to_ai(episode_id: str, user: Annotated[User, Depends(get_user_info)], ai_users: List[str]):
    """ Send EGM to AI """
    
    logger.info(f"Tentative d'envoi de l'EGM au serveur IA pour l'épisode {episode_id}")
    
    for ai_user in ai_users:
        logger.info(f"Envoi de l'EGM à l'IA {ai_user}")
        async with httpx.AsyncClient() as client:
            response = await client.post(f"http://{ai_user}/:some-port/egm-analysis", content=episode_id)
            logger.info(f"Réponse de l'IA {ai_user}: {response.json()}")
